...
function composeMelody(chords: number[][], slot: number, quiet: number): CleanScore['melody'] {
  ...
  return {
    notes: steps.join(' '),
    cycleSeconds: Math.round(lerp(40, 60, quiet)),
...
export function compose(inputs: RoomInputs): CleanScore {
  ...
  return {
    ...
    chords,
    chordSeconds: Math.round(lerp(56, 96, quiet)),
    ...
    melody: composeMelody(chords, Math.floor(inputs.at / SCORE_INTERVAL_S), quiet),
  };
}
