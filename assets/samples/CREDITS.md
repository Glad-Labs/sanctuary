# Sample credits

All recordings here are CC0 or public domain, converted to mono 96 kbps MP3
and loudness-normalized.

- `bass_*`, `cello_*`, `viola_*`, `violin_*`, `flute_*`, `cymbal_bow_*`,
  `gong_soft`, `vibra_ring`: VSCO 2 Community Edition, Versilian Studios,
  CC0 1.0. https://github.com/sgossner/VSCO-2-CE
- `bowl_1`: "SingingBowl1.ogg" by BambooBeast, public domain, Wikimedia Commons.
- `gong_55`: "Gong55.ogg" by stephan, CC0, Wikimedia Commons.
- `gong_vibrant`: "Gong or bell vibrant (short).ogg", PDSounds, CC0, Wikimedia Commons.
- `breeze`: "Gentle breeze and birds singing.ogg" by ezwa, public domain, Wikimedia Commons.
- `ocean`: "Ocean Waves on a Tropical Beach.ogg" by Jarrod Stanley, CC0, Wikimedia Commons.
